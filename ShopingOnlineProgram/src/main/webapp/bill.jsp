...
try
{
	...
	int sno=0;
	 ...
	 Connection con=ConnectionProvided.getCon();
  	 ...
  	 ResultSet rs=st.executeQuery("select sum(total) from carte where email='"+email+"' and status='bill'");
     ...
     ResultSet rs1=st.executeQuery("select * from users inner join carte where carte.email='"+email+"' and carte.status='bill'");
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs2=st.executeQuery("select * from carte inner join product where carte.product_id=product.id and carte.email='"+email+"' and carte.status='bill'");
    while(rs.next())
	 {
