...
try
{
	Connection con=ConnectionProvided.getCon();
 	...
 	PreparedStatement ps1=con.prepareStatement("update carte set address=?,city=?,state=?,country=?,mobileNumber=?,orderDate=now(),deliveryDate=DATE_ADD(orderDate,INTERVAL 7DAY),paymentMethod=?,transactionId=?,status=? where email=? and address is NULL");
